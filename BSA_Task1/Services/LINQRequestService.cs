...
namespace BSA_Task1.Services
{
    class LINQRequestService
    {
        ...
        public Collection<(int id, string name, List<IGrouping<int?, User>> users)> Task4() =>
           new Collection<(int id, string name, List<IGrouping<int?, User>> users)>(teams
               ...
               .GroupBy(user => user.TeamId).ToList()
               )).ToList());
